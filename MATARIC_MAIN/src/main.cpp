...
void spin(int direction) {
  int wheelStepsForDistance = (800 / wheelCirc) * ((robotDiam * PI)); // (steps per rotation / distance per rotation) * desired distance
...
void turn(int direction) {
    ...
    if (direction == 0){
    stepperRight.moveTo(wheelStepsForDistance);
    stepperLeft.moveTo(wheelStepsForDistance/2);
    stepperRight.setMaxSpeed(300);//set right motor speed
    ...
    stepperRight.moveTo(wheelStepsForDistance/2);
    stepperLeft.moveTo(wheelStepsForDistance);
...
void forward(int distance) {
  ...
  // Calculates the distance in cm to encoder pulses (40 pulses per rotation)
  float encoderPulsesForDistance = (40 / wheelCirc) * distance;

  // Start moving to goal;
  stepperRight.moveTo(100000);
  stepperLeft.moveTo(100000);
  ...
  while(1){
    ...
    // Calculates error based on where encoder needs to be
    int errorRight = (encoderPulsesForDistance - encoder[RIGHT]);
    int errorLeft = (encoderPulsesForDistance - encoder[LEFT]);

    Serial.println(errorRight);
    Serial.println(errorLeft);

    // Find velocity
    float veloRight = 800 *  (errorRight/encoderPulsesForDistance);
    float veloLeft  = 800 * (errorLeft/encoderPulsesForDistance);

    // Sets velocity
    stepperRight.setSpeed(veloRight + 200);
    stepperLeft.setSpeed(veloLeft + 200);
...
void moveCircle(int diam, int dir) {
  ...
  int innerCirc = innerDiam * PI;
  int outterCirc = outterDiam * PI;

  int innerTicks = (800 / wheelCirc) * (innerCirc); // (steps per rotation / distance per rotation) * desired distance
  int outterTicks = (800 / wheelCirc) * (outterCirc); // (steps per rotation / distance per rotation) * desired distance
  ...
  if (dir == 0){
    ...
    stepperLeft.moveTo(innerTicks);
    stepperRight.setMaxSpeed(rightSpeed);//set right motor speed
    stepperLeft.setMaxSpeed(leftSpeed);//set left motor speed
    stepperRight.runSpeedToPosition();//move right motor
    stepperLeft.runSpeedToPosition();//move left motor
    runToStop();//run until the robot reaches the target
  }
